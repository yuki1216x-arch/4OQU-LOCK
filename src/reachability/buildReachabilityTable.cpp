...
int main(int argc, char *argv[]) {
  ...
  if(iteration == 0) {
    ...
    string base_filename = "game_length_" + base[vision];
    ...
    Table parent_table(17 - iteration, read_filename_str.c_str(), argv[3], placement_count[vision][17 - iteration]);
    Table child_table(16 - iteration, read_filename_str_child.c_str(), argv[4], placement_count[vision][16 - iteration]);
    Posi p;
    ...
    for(unsigned long long int i = 0ULL; i < max_haiti; i++) {
      assert(nreachable <= max_haiti_child);
    ...
    cout << "before_outtable" << endl;
    {
      OutTable out_table(16 - iteration, read_filename_str_child.c_str(), placement_count[vision][16 - iteration]);
      for(unsigned long long int i = 0; i < placement_count[vision][16 - iteration]; i++) {
