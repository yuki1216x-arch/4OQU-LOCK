...
constexpr int nworker1 = 24;
constexpr int nworker2 = 4;
...
condition_variable cv_worker1;
condition_variable cv_worker2;
mutex mtx;
...
public:
  ...
  void set_processing(int nchild, int game_length, int dicision_case) noexcept {
    m_nchild = nchild;
    m_game_length = game_length;
    m_dicision_case = dicision_case;
  }
  ...
  void set(int nchild_opp[][max_belief_state], int num_of_un_opp[][max_belief_state],
	...
	// if(num_of_un_opp[i][j] <= 0 || num_of_un_opp[i][j] > 12) cout << "id = " << m_id << ", unknown_opp = " << num_of_un_opp[i][j];
  ...
  int get_nchild() const noexcept { return m_nchild; }
  int get_game_length() const noexcept { return m_game_length; }
  int get_num_of_un() const noexcept { return m_num_of_un; }
  int get_dicision_case() const noexcept { return m_dicision_case; }
...
deque<Work *> deq_processing;
deque<Work *> deq_output;

static void boss(int iteration, int vision, Table& parent_table, const char* write_file_name) noexcept {
  cout << "boss" << endl;

  unsigned long long int max_placement = placement_count[vision][16 - iteration];
  unsigned long long int count_input = 0ULL;

  int nstack_work_idle = deq_input_size + deq_output_size + nworker1;
  ...
  while(true) {
    ...
    if((deq_input.size() < deq_input_size) && (deq_output.size() < deq_output_size) && count_input < max_placement && nstack_work_idle >= 1) {
	...
	for(unsigned int workid = 0; workid < deq_tmp.size(); workid++) {
	  ...
	  int nchild = deq_tmp[workid]->get_nchild();
	  int game_length = deq_tmp[workid]->get_game_length();
	  int num_of_un = deq_tmp[workid]->get_num_of_un();
	  int dicision_case = deq_tmp[workid]->get_dicision_case(); // 0: mm_me, 1: mm_ee, 2: me_mm, 3: me_em
	  ...
	  if(nchild == -1 || nchild == -4) {
	    if(nchild == -1) al_win_num++;
	    if(nchild == -4) {
	      if(dicision_case == 0) mm_me_win++;
	      else if(dicision_case == 1) mm_ee_win++;
	      else if(dicision_case == 2) me_mm_win++;
	      else {
		assert(dicision_case == 3);
		me_em_win++;
	      }
	    }
	    assert(game_length >= 0 && game_length <= 16 - iteration);
	    parent_table.set(id, v_win, game_length);
	    //cout << "win" << endl;
	    nwin++;
	    win_length_counts[game_length]++;
	  } else if(nchild == -2 || nchild == -5) {
	    if(nchild == -2) al_lose_num++;
	    if(nchild == -5) {
	      if(dicision_case == 0) mm_me_lose++;
	      else if(dicision_case == 1) mm_ee_lose++;
	      else if(dicision_case == 2) me_mm_lose++;
	      else {
		assert(dicision_case == 3);
		me_em_lose++;
	      }
	    ...
	    assert(game_length >= 0 && game_length <= 16 - iteration);
	    parent_table.set(id, v_lose, game_length);
	    //cout << "lose" << endl;
	    nlose++;
	    loss_length_counts[game_length]++;
	  } else if(nchild == -8) {
	    if(dicision_case == 0) all_draw++;
	    parent_table.set(id, v_draw, 0ULL);
	    //cout << "draw" << endl;
	    ndraw++;
	  } else if(nchild == -6) {
	    if(dicision_case == 0) mm_me_ntlose++;
	    else if(dicision_case == 1) mm_ee_ntlose++;
	    else if(dicision_case == 2) me_mm_ntlose++;
	    else {
	      assert(dicision_case == 3);
	      me_em_ntlose++;
	    }
	    ...
	    if(dicision_case == 0) mm_me_ntwin++;
	    else if(dicision_case == 1) mm_ee_ntwin++;
	    else if(dicision_case == 2) me_mm_ntwin++;
	    else {
	      assert(dicision_case == 3);
	      me_em_ntwin++;
	    }
  ...
  cout << "before_outtable" << endl;
  {
    ...
    OutTable out_table(16 - iteration, write_file_name, max_placement, bits_per_entry);
  ...
  cv_worker1.notify_all();
  cv_worker2.notify_all();
}

static void worker1(int iteration, int vision, const ZDD& zdd_parent, const ZDD& zdd_child, const ZDD& zdd_child_opp) noexcept {
  Work *w;
  while(true) {
    unique_lock<mutex> lck(mtx);

    cv_worker1.wait(lck, [&](){ if(0 < deq_input.size()) return true;
    ...
    unique_lock<mutex> lck2(mtx_processing);
    deq_processing.push_front(w); // 1つのスレッドしか触っちゃいけない
    lck2.unlock(); // ロック解除
    cv_worker2.notify_one(); // cv_bossで現在waitをcallしているスレッド1つに信号が行く。
  }
}

static void worker2(int iteration, int vision, const Table& child_table, const Table& opp_child_table) {
  Work *w;
  while(true) {
    unique_lock<mutex> lck2(mtx_processing); //ロック
    cv_worker2.wait(lck2, [&](){ if (0 < deq_processing.size()) return true;
	return flag_worker_quit; }); // 仕事がある or (表に記入すべきものがあり、表がkeepされていない) or 仕事が全て終わっている になるまで待つ
    if(flag_worker_quit) break;
    assert(0 < deq_processing.size()); // 仕事があるなら
    w = deq_processing.back();
    deq_processing.pop_back();
    lck2.unlock();

    int nchild = w->get_nchild();
    int num_of_un = w->get_num_of_un();
	...
	for(int j = 0; j < num_of_action; j++) { //合法手の数だけループ
	  ...
	  if(j == num_of_action - 1 && iteration % 2 == vision) {
	    if(nlose_plan_num > 0 || draw_plan_num > 0) {
	      if(already_decided != 1) {
		notlose_flg = 1;
		if(which_table == 0) dicision_case = 0;
		else dicision_case = 1;
	      }
	      already_decided = 1;
	    }
	  }
	...
	if(already_decided == 0) {
	  if(win_plan_num == num_of_action) {
	    ...
	    if(which_table == 0) dicision_case = 2;
	    else dicision_case = 3;
	    notlose_flg = 1;
	  }
	}      
	if(game_val < 0) {
    ...
    if(nchild > 0) { //後半のループでは必敗、勝無しを判定
      ...
      w->get_opp(nchild_opp, num_of_un_opp, array_id_opp);
      for(int which_table = 0; which_table < 2; which_table++) {
	int opp_win_count = 0;
    ...
    if(nchild > 0) nchild = -9;

    int game_length = 0;
    if(nchild == -1 || nchild == -4) game_length = win_game_length;
    if(nchild == -2 || nchild == -5) game_length = lose_game_length;
    w->set_processing(nchild, game_length, dicision_case);
    
    unique_lock<mutex> lck(mtx); // ロック（deq_outputに触るため）
...
int main(int argc, char *argv[]) {
  ...
  thread th_boss(boss, iteration, vision, std::ref(parent_table), read_filename_str.c_str());
  ...
  thread th_worker1[nworker1];
  for(int workerid = 0; workerid < nworker1; workerid++) {
    th_worker1[workerid] = thread(worker1, iteration, vision, std::cref(*zdd_parent), std::cref(*zdd_child), std::cref(*zdd_child_opp)); // ここでworker()を呼び出す
  }

  thread th_worker2[nworker2];
  for(int workerid = 0; workerid < nworker2; workerid++) {
    th_worker2[workerid] = thread(worker2, iteration, vision, std::cref(child_table), std::cref(opp_child_table));
  }

  // 終了処理
  th_boss.join();
  for(int workerid = 0; workerid < nworker1; workerid++) {
    th_worker1[workerid].join();
  }
  for(int workerid = 0; workerid < nworker2; workerid++) {
    th_worker2[workerid].join();
  }
